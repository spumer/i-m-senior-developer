...
неочевидный trade-off. Мера полезности — холодное чтение: комментарий пишется
тому, кто откроет файл через полгода без задачи, плана и диффа перед глазами.
Четыре класса комментариев — смелл (шум, увеличивающий
когнитивную нагрузку): doc-ссылки на внутренние секции (`see DESIGN §9.4`),
changelog в коде (`no longer reads DATABASE_URL`), пересказ очевидного
(`// increment counter`), рассказ о своей работе — то, что адресовано человеку
в чате, а не будущему читателю кода (например, почему отступил от плана).

Нормативный текст — примеры ❌/✅, allowlist, пограничные случаи — живёт в
одном месте: `references/05-comment-style.md`; эта врезка — напоминание, не
копия. Tracking-id (`FEAT-XXXX`) покрыт отдельно плагином `sdlc`.
