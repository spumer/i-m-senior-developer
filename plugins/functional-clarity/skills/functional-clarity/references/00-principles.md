...
На границе контекстов — слоёв, модулей, сервисов, систем — у имени есть ещё и владелец: словарь объявления принадлежит контексту, который его объявил. Порт, событие, сигнатура или поле схемы, названные действием или механизмом потребителя, — дефект границы, а не стилистика; критерий — тест подстановки (замени контрагента за границей — объявление должно остаться точным). Норматив: `references/06-boundary-vocabulary.md`.
...
Комментарий объясняет «почему» (причина, инвариант, trade-off), а не
пересказывает код, не ссылается на внутренние доки, не хранит историю
изменений и не рассказывает о работе автора — это адресовано человеку в чате,
а не читателю кода. Нормативный текст — классы смеллов с ❌/✅, allowlist,
пограничные случаи — в `05-comment-style.md`; здесь только напоминание.
