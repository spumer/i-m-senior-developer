...
Определи каталог фичи по переданному каталогу или файлу. Актуальный план выполнения — `PLANNER_EXECUTION.md` в этом каталоге. После проверки сохрани канонические абсолютные пути как `feature_directory`, `execution_path` и `architecture_path`.
...
## 2. Разреши проектный контекст до работы

После успешной проверки каталога фичи, плана и архитектуры вызови помощник состояния:
...
- `0` — сохрани поле `path` из JSON как `planner_context_path`. Это единственный путь, по которому команда читает контекст проекта;
- `3` — не запускай ни одного рабочего вызова. Покажи человеку `status`, каждый путь из `searched` в порядке проверки и точную команду восстановления:

  ```text
  /planner:plan <разрешённый-путь-к-архитектуре>
  ```

- `64` — вызов помощника составлен неверно; исправь его и повтори разрешение до любого рабочего вызова.

Проверка не создаёт и не изменяет контекст. При коде `3` не сканируй проект и не выбирай роль вместо остановки.

## 3. Проверяй свежесть перед каждым вызовом агента
...
Перед первым рабочим вызовом после успешной проверки покажи человеку `work_hint.message`. Затем покажи все `work_hint.paths`: путь, его `scope`, а для проверенного пути — полный `commit` и `committed_at`; для непроверенного — `reason`. Это сведения, а не новый гейт. В частности, `outputs_changed` означает лишь изменение хотя бы одного объявленного выхода после сборки: не останавливай цикл, не пропускай фазу и не объявляй план выполненным. При следующих гейтах не повторяй подсказку, пока объект `work_hint` не изменился. Коды свежести обрабатывай по правилам выше при любом статусе подсказки.
...
## 4. Владей путями и формой передачи

Прочитай план выполнения. Сопоставляй роли только с действующими агентами из `planner_context_path` §1; не придумывай имена. Используй каталог фичи и правила хранения из §5.
...
После чистого завершения всех фаз реализации зарезервируй путь `DOCUMENTATION-NN.md` подкомандой `reserve-report` с видом `documentation`. Перед вызовом активной документационной роли из `planner_context_path` §1 повторно проверь свежесть. Передай только разрешённые в плане пути документации, точный путь зарезервированного пустого файла полного отчёта, guard идентификатора и договор:
