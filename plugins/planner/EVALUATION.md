...
- **[Запуск]** — проверено фактическим запуском CLI в Claude Code `2.1.233`.
- **[CLI 2.1.233]** — прочитано из встроенного reference и реализации установленного CLI. Положительный eval-run в текущей организации недоступен из-за закрытого early-access gate.
...
`claude plugin eval --help` и `claude plugin eval init --help` при этом доступны. Команда `init --bare` до создания шаблона также останавливается на сообщении о раннем доступе.

### Что делать при закрытом гейте

1. Обновить Claude Code.
2. Закрыть текущую сессию и начать новую.
3. Проверить, включена ли организация в early access.
4. Для Bedrock, Vertex, Foundry, LLM gateway, custom `ANTHROPIC_BASE_URL` и окружений, где отключён nonessential traffic или feature-flag fetch, получить enablement-переменную у контакта Anthropic. Имя переменной нельзя угадывать.
5. Повторить самотест в пустом каталоге.

Не пытайтесь диагностировать закрытый гейт созданием кейсов в репозитории: при закрытом доступе они не будут прочитаны.
...
- в исследованной среде установлена `2.1.233`.
...
По умолчанию CLI ищет кейсы в `<plugin-root>/evals/`. В текущем CLI `2.1.233` опции `--eval-dir` нет.
...
Помощник `product_state.py` уже покрыт 47 обычными тестами. Для него `claude plugin eval` не нужен: это детерминированный код с обычным машинным контрактом.
...
Ниже пример для документации. Сам каталог `evals/` этим файлом не создаётся.
...
- Формат кейсов, грейдеров, sandbox и JSON v1 — встроенный reference и реализация Claude Code `2.1.233` — **[CLI 2.1.233]**.
