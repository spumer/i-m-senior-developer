...
В текущей среде гейт закрыт на каждой проверенной версии — от `2.1.228` до `2.1.234`:

```text
`plugin eval` is currently in early access
```

Это не регресс отдельной сборки: обновление CLI гейт не открывает.
...
### Что проверяется при закрытом гейте
