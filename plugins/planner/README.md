...
`/planner:plan-do-workflow <каталог-фичи>` — отдельная экспериментальная команда для текущего плана выполнения.

Заявленный договор такой. Команда предварительно проверяет каталог и обязательные файлы, затем запускает workflow `planner:plan-do-workflow` из `workflows/plan-do-workflow.js`. Обычный цикл `/planner:plan-do` не меняется; экспериментальная команда не вызывается автоматически и при отказе на обычную не переключается. Итог возвращает состояние, число рабочих вызовов, число проверок свежести, забронированные пути и пути записанных отчётов, без полного текста отчётов.

Этот договор пока не подтверждён запуском. Ни один прогон не дошёл до исполнения тела workflow: среда отклоняет запуск динамического workflow сообщением `Review dynamic workflow before running`. Наблюдено только то, чему исполнение тела не нужно, — отказ команды без аргумента с полной формой вызова и работа обычного `/planner:plan-do` прежним путём. Пока это так, на итог экспериментального режима полагаться нельзя.
