...
- **Same write reachable from two entrypoints without a shared service.** When a mutation is exposed both as a REST endpoint AND a second entrypoint (MCP tool, CLI command, admin action, background job), implementing the write twice guarantees drift — different filters, one fires the side-effect/event engine and the other skips it, logs diverge. Design rule: one shared service function owns the write and its side effects; every interface is a thin adapter that validates and delegates. Name that function in the Hand-off Contracts table with each interface as a caller of the same row, never two independent rows.
