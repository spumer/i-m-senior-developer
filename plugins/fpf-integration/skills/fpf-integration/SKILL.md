...
> **Доступ к спецификации:** глубокий разбор аксиом опирается на канонический
> текст FPF. Источник берётся по приоритету: **сначала FPF-справочник через MCP**
> (если он подключён — самый авторитетный: точные ID, цитаты, живая свежесть),
> **иначе локальная копия `FPF-Spec.md`** (~9.5 МБ, не входит в плагин), **иначе
> облегчённый режим** (Quick Reference + glossary — работает всегда). Проверка
> источника — не «при активации», а в момент первого обращения к спеке: см.
> **«Протокол доступа к FPF»** ниже.
...
### Уровень 3 — облегчённый режим (нет ни MCP, ни копии)

Если недоступны и MCP, и копия — предупредить пользователя: *«FPF недоступен ни через
MCP-справочник, ни локальной копией — глубокий разбор по секциям невозможен; работаю
в облегчённом режиме (Quick Reference + glossary + audit-patterns + role-templates).
Чтобы включить полный режим: подключить FPF-справочник как MCP, либо скачать спеку из
`https://github.com/ailev/FPF` в `~/.claude/knowledge/fpf/FPF-Spec.md` (проще всего —
`scripts/fetch-fpf-spec.sh`)»*. Спросить: продолжить облегчённо или сначала настроить
доступ. **Не выдумывать содержимое секций спеки по памяти** — опираться только на
встроенные reference-файлы (A.10: claim without source = opinion).
