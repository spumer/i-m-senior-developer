...
- **Раздел `## Профиль зрелости`** (в package-adequacy, автор — детерминированный скрипт ниже, аннотатор — строка критика) несёт: (1) вычисленный уровень L0–L4 + определение (сочетание требований этого уровня); (2) компонентный чек-лист ✓ / ✗ / «✓ presence / критик: подтвердить-слабый» с сигнал-локусом на каждый компонент; (3) сводку D-оси — референс на таблицу D1–D11 выше в этом же файле (min, floor-fragile), НЕ копию; (4) список «доработать next» с привязкой к фазе `dpf-authoring`, закрывающей пробел; (5) эхо строки `maturity-critic`. Не дублирует D1–D11 (A.11 — они уже в этом файле). Уровень (спектр L0–L4) — не то же самое, что статус допуска выше (пол L1); A.7 держит различие.
- **Потолок зрелости** — критик добавочно выносит РОВНО ОДНУ машинно-читаемую строку `> maturity-critic: L<n> confirmed (guardian, <date>); weak-components: [<id>]` (id строго из набора: `canon-patterns`, `PFR-network`, `refresh-route`, `acceptance-cases`, `support-maps`, `executable-sync`) — подтверждённый по содержанию потолок уровня + пометка формально-присутствующих, но слабых компонентов. Для L0–L2 строка не обязательна (структурного сигнала + D-таблицы достаточно), но выноси её всегда при состоявшемся прогоне — она дешёвая и снимает неоднозначность для скрипта; для L3/L4 — обязательна (её отсутствие капит структурный кандидат L3/L4 до L2, fail-closed).
- **Детерминированный слой поверх критика** — `assets/maturity.py` (`maturity.py <package-dir> [--json] [--write-profile]`) читает package-adequacy + канон-скелет DPF.md (read-only), считает presence компонентов + D-арифметику (структурный кандидат уровня); критик выше даёт substance-потолок строкой `maturity-critic` — скрипт содержание не судит, критик арифметику не считает (A.7: роль ≠ реализация). Финальный уровень для L3/L4 = `min(структурный кандидат, критик-потолок)`. `--write-profile` рендерит раздел «Профиль зрелости» в package-adequacy; без флага — только печатает сводку.
