...
FPF — читать ЖИВЬЁМ, не выжимки: Grep по ${FPF_SPEC} (паттерн вида '^## E\\.4\\.DPF', НЕ по номеру строки), затем Read вокруг матча.
...
Прочитай ${dirOf(c)}/references/sota-research.md. Сверь форму с FPF E.4.DPF:3, E.4.PFR, A.2.6, B.5.2.1, A.11 — Grep по спеке.
...
Прочитай ${dirOf(c)}/DPF.md и references. Сверь CC-DPF.1–9 (E.4.DPF:7) и анти-паттерны (E.4.DPF:8) — Grep по спеке ('^#* *E\\.4\\.DPF').
Затем оцени ПАКЕТ по E.4.DPF.DA (Grep '^#* *E\\.4\\.DPF\\.DA'): сперва подпроход формы PFM1–PFM11, затем таблица всех 11 координат D1–D11 — каждая строка: значение 0–5 | краткое обоснование | evidence-locus (конкретная секция/файл) | repair-предложение. Пол = 4 (опора ролей). Средним баллом паттернов подменять НЕЛЬЗЯ (CC-DPFDA.4).
