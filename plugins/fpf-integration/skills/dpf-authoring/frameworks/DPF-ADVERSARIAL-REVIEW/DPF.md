...
> **Потолок зрелости (maturity-ceiling).** На Фазе 6, после D1–D11 + статуса, критик добавочно выносит в `package-adequacy-<date>.md` ровно одну машинно-читаемую строку `> maturity-critic: L<n> confirmed (guardian, <date>); weak-components: [...]` — подтверждённый по СОДЕРЖАНИЮ потолок уровня зрелости пакета + пометку формально-присутствующих, но слабых компонентов. Строку читает детерминированный maturity-слой (он считает presence компонентов + арифметику D); критик даёт только substance-потолок, арифметику уровня не считает (A.7: роль ≠ реализация). Набор паттернов и статус-протокол это не меняет — строка добавочна к выходу Фазы 6. Формат и правило вывода — `apply-prompt.md`, Режим B, шаг 6.
