...
FPF — читать ЖИВЬЁМ, не выжимки, по приоритету источника:
  ...
  (3) НЕТ НИ MCP, НИ локальной копии ${FPF_SPEC} → СТОП: не авторь по памяти (это claim without source, A.10). Верни ok=false, gate_passed=false, notes='FAIL-FAST: FPF недоступен (ни MCP fpf_reference, ни локальная копия FPF-Spec.md) — авторинг DPF по памяти запрещён; подключи FPF MCP или скачай спеку и повтори' и остановись.
