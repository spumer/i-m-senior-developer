...
вызывают скилл. Предел `$6` оставляет запас над стоимостью зелёного прогона;
фактическая сумма видна в `aggregate-result.json` прогона.
...
повторы — живут в его `aggregate-result.json` и в документацию не переносятся.
Вопрос о пороге гейта закрыт решением удержать строгий порог — основание
записано на странице [принятых решений](../decisions.md).
