...
маршрутизация в `product-discovery` не срабатывала, на `opus` те же кейсы
вызывают скилл — история наблюдений в [отчёте planner](../reports/planner.md).
Предел `$6` оставляет запас над стоимостью зелёного прогона; актуальная сумма —
там же, в отчёте.
...
Числа последнего прогона — стоимость, длительность, оценки кейсов и успешные
повторы — живут в одном месте: [отчёте planner](../reports/planner.md).
Вопрос о пороге гейта закрыт решением удержать строгий порог — основание
записано на странице [принятых решений](../decisions.md); актуальное состояние
прогона и история — в отчёте.
...
Каталог кейсов задаётся опцией `--eval-dir <dir>` — именем каталога внутри
плагина; сверен запуск `claude plugin eval --help` на CLI `2.1.239`. Без опции
используется `evals/`, если манифест плагина не задаёт другое имя полем
`experimental.evals`; результаты пишутся в `<этот-каталог>/results/`.
